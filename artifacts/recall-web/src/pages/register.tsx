    ...
    <div className="min-h-screen flex items-center justify-center bg-[var(--surface)] p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-[420px] glass rounded-2xl border border-[var(--outline-variant)] shadow-[0_16px_48px_rgba(163,166,255,0.06)] relative z-10 p-8 sm:p-10">
        <div className="text-center mb-10 space-y-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white mb-2" style={{ fontFamily: "var(--app-font-display)" }}>Node Creation</h1>
            <p className="text-sm text-[var(--on-surface-muted)]">Establish a new construct in the archive</p>
          </div>
        </div>

        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-1">
            <label className="label-caps text-[var(--on-surface-muted)] ml-1">Designation (Username)</label>
            <input
              type="text"
              value={username}
          ...
          <Button type="submit" className="w-full h-12 gradient-btn rounded-lg mt-2 text-base font-bold tracking-wide" disabled={registerMutation.isPending}>
            {registerMutation.isPending ? <Loader2 className="h-5 w-5 animate-spin" /> : "Initialize Node"}
          </Button>
        </form>
        ...
        <div className="mt-8 text-center border-t border-[var(--outline-variant)] pt-6">
          <p className="text-sm text-[var(--on-surface-muted)]">
            Existing construct?{" "}
            <Link href="/login" className="text-[var(--primary)] hover:text-[var(--secondary)] font-medium transition-colors">
