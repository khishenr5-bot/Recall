    ...
    mutation: {
      ...
      onError: (error) => {
        toast({
          ...
          description: (error as any).message || "Invalid email or password",
          variant: "destructive",
        });
      },
    },
    ...
    <div className="min-h-screen flex items-center justify-center bg-[var(--surface)] p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-[420px] glass rounded-2xl border border-[var(--outline-variant)] shadow-[0_16px_48px_rgba(163,166,255,0.06)] relative z-10 p-8 sm:p-10">
        <div className="text-center mb-8 space-y-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-[var(--on-surface)] mb-1" style={{ fontFamily: "var(--app-font-display)" }}>Welcome back</h1>
            <p className="text-sm text-[var(--on-surface-muted)]">Sign in to your Recall.ai account</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button type="submit" className="w-full h-12 gradient-btn rounded-lg mt-2 text-base font-semibold" disabled={loginMutation.isPending}>
        ...
        <div className="mt-6 text-center border-t border-[var(--outline-variant)] pt-5">
          <p className="text-sm text-[var(--on-surface-muted)]">
            ...
            <Link href="/register" className="text-[var(--primary)] hover:underline font-medium transition-colors">
              Sign up
            </Link>
          </p>
        </div>
