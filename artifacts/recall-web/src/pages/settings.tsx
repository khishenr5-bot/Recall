...
export default function Settings() {
  ...
  return (
    <div className="container mx-auto px-4 py-10 max-w-4xl space-y-10">
      <div>
        <h1 className="text-[3rem] font-bold tracking-tight leading-none text-white" style={{ fontFamily: "var(--app-font-display)" }}>System Configuration</h1>
        <p className="text-[var(--on-surface-muted)] mt-3 text-lg">Manage your node parameters and data persistence.</p>
      </div>

      <div className="bg-[var(--surface-high)] p-8 rounded-[0.5rem] border-none">
        <h2 className="text-xl font-bold text-white mb-6" style={{ fontFamily: "var(--app-font-display)" }}>Profile Matrix</h2>
        <form onSubmit={handleSaveProfile} className="space-y-6">
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="label-caps text-[var(--on-surface-muted)]">Designation</label>
              <input 
            ...
            <div className="space-y-2">
              <label className="label-caps text-[var(--on-surface-muted)]">Identity</label>
          ...
          <div className="space-y-2">
            <label className="label-caps text-[var(--on-surface-muted)]">Translation Core</label>
          ...
          <Button type="submit" className="gradient-btn px-8" disabled={updateProfileMutation.isPending}>
            {updateProfileMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Apply Changes
          </Button>
        </form>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-[var(--surface-high)] p-8 rounded-[0.5rem]">
          <h2 className="text-xl font-bold text-white mb-2 flex items-center gap-2" style={{ fontFamily: "var(--app-font-display)" }}><Link2 className="h-5 w-5 text-[var(--primary)]" /> Notion Uplink</h2>
          <p className="text-[var(--on-surface-muted)] text-sm mb-6">Synchronize cognitive blocks directly to a Notion database.</p>
          ...
          {notionStatus?.connected ? (
            <div className="space-y-4">
              <div className="flex items-center gap-3 p-4 bg-[#50fa7b]/10 border border-[#50fa7b]/30 rounded-lg">
                ...
                <p className="text-sm text-[#50fa7b] font-medium">Uplink Established {notionStatus.workspaceName ? `(${notionStatus.workspaceName})` : ""}</p>
              </div>
              <div className="flex gap-3">
                <Button onClick={() => {}} disabled={notionSyncing} className="bg-[var(--surface-bright)] hover:bg-[var(--surface-highest)] text-white border border-[var(--outline-variant)]">
                  {notionSyncing ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Zap className="h-4 w-4 mr-2" />} Force Sync
                </Button>
        ...
        <div className="bg-[var(--surface-high)] p-8 rounded-[0.5rem]">
          <h2 className="text-xl font-bold text-white mb-2 flex items-center gap-2" style={{ fontFamily: "var(--app-font-display)" }}><Download className="h-5 w-5 text-[var(--secondary)]" /> Data Extraction</h2>
          <p className="text-[var(--on-surface-muted)] text-sm mb-6">Export your neural archive to standard formats.</p>
