...
- Pages: Home (/), Saved Library (/saved), Highlights (/highlights), Insights (/insights), Knowledge Graph (/graph), Creator Tools (/create), Public Share (/share/:token), Settings (/settings), Auth (/login, /register)
...
- `users` — email, password_hash, plan (free/pro), monthly_saves_count, saves_limit, preferred_language
- `saved_articles` — title, verdict, bullets (jsonb), recall_score, credibility_score, share_token, spaced repetition fields
